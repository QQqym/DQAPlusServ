package com.cn.chinamobile.redis;

import com.cn.chinamobile.util.ConfigUtils;
...
public class JedisUtils {
	...
	public static void InitPorpertiesFile(String propFile) {
		if (prop == null) {
			prop = ConfigUtils.getConf(propFile);
		}
	}
	...
	private static void initPool() {
		...
		host = ContentInfo.REDIS_HOTS;
		port = Integer.parseInt(ContentInfo.REDIS_PORT);
		password = "";
		pool = new JedisPool(conf, host, port, 60 * 1000);
		System.out.println("redis info "+host +":"+ port);
		//pool = new JedisPool(conf, host, port, 60 * 10000, password);
	}
